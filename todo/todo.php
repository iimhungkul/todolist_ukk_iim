...
<?php 
  ...
  if (isset($_GET['pesan_edit'])) {
    ?>
    <div class="alert alert-<?php echo $_GET['pesan_edit']=='berhasil'?'success':'danger' ;?> alert-dismissible fade show" role="alert"style="color:white;">
...
<!-- Checkbox -->
  ...
  function cekForm() {
    if (tugasInput.val().length < 5 || !deadlineInput.val()) {
      tombolSimpan.prop("disabled", true).css("background-color", "#494b5c"); // Warna saat belum valid
      ...
      tombolSimpan.prop("disabled", false).css("background-color", "#203bd4"); // Warna saat valid
      errorTugas.text("");
      errorDeadline.text("");
    }
  }
...
<button style="float: right; border: 2px solid white; color:white;" type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
...
<div class="row d-flex justify-content-end mb-2">
  <div class="col-2" >
    <input type="text" name="cari" placeholder="Cari" class="col-12 form-control btn btn-primary">
  </div>
  <div class="col-1" >
    <button type="submit" class="10 form-control btn btn-primary" title="cari" style="float: right; border: 2px solid white; color:white;">
...
<?php if (isset($_POST['cari']) && !empty(trim($_POST['cari']))) { ?>
  <div class="d-flex justify-content-right mb-2">
    <a href="index.php?halaman=todo" class="btn btn-primary border-light px-3"  >
...
<nav aria-label="Page navigation example">
  <ul class="pagination" >
    <li class="page-item"><a class="page-link" href="<?php if ($star - 5 < $jlhdata) echo "index.php?halaman=todo&star=" . 
     ($star - 5); ?>"style="color:body;">Previous</a></li>
    <?php 
    $page=0;
      for ($i=0; $i < $perulangan; $i++) { 
        ?>
         <li class="page-item"><a class="page-link" href="index.php?halaman=todo&star=<?= $page ?>"
         style="color:body;"><?=$i+1?></a></li>
        <?php 
        $page+=5;
      }
     ?>
     ...
     ($star + 5); ?>" style="color:body;">Next</a></li>
  </ul>
</nav>
</div>